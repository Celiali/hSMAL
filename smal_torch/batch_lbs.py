...
def batch_skew(vec, batch_size=None, device=None):
    ...
    res = torch.Tensor(np.zeros(out_shape[0])).to(device)
...
def batch_lrotmin(theta):
    ...
    Returns
      diff_vec : `Tensor`: N x 315 rotation matrix of 35=(K-1) joints with identity subtracted.,
    """
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base = False, device= None):
    """
    ...
    # ---
    Js_w0 = torch.cat([Js, torch.zeros([N, 36, 1, 1]).to(device)], 2)
